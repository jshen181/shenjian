 ...
 *
 */
if ( ! class_exists( 'ZMOP_Metabox' ) ) {
  class ZMOP_Metabox extends ZMOP_Abstract{
    ...
    public function add_metabox_classes( $classes ) {
      ...
      if ( ! empty( $this->post_formats ) ) {
        ...
        foreach ( $this->post_formats as $format ) {
          $classes[] = 'zmop-post-format-'. $format;
        }

        if ( ! in_array( $saved_post_format, $this->post_formats ) ) {
          $classes[] = 'zmop-metabox-hide';
        } else {
          $classes[] = 'zmop-metabox-show';
        }
      ...
      if ( ! empty( $this->page_templates ) ) {
        ...
        $classes[] = 'zmop-page-templates';

        foreach ( $this->page_templates as $template ) {
          $classes[] = 'zmop-page-'. preg_replace( '/[^a-zA-Z0-9]+/', '-', strtolower( $template ) );
        }
    ...
    public function add_meta_box_content( $post, $callback ) {
      ...
      $errors    = ( is_object ( $post ) ) ? get_post_meta( $post->ID, '_zmop_errors_'. $this->unique, true ) : array();
      ...
      if ( is_object ( $post ) && ! empty( $errors ) ) {
        delete_post_meta( $post->ID, '_zmop_errors_'. $this->unique );
      }

      wp_nonce_field( 'zmop_metabox_nonce', 'zmop_metabox_nonce'. $this->unique );

      echo '<div class="zmop zmop-metabox'. esc_attr( $theme ) .'">';

        echo '<div class="zmop-wrapper'. esc_attr( $show_all ) .'">';

          if ( $has_nav ) {

            echo '<div class="zmop-nav zmop-nav-'. esc_attr( $nav_type ) .' zmop-nav-metabox">';
              ...
              foreach ( $this->sections as $section ) {
                ...
                $tab_error = ( ! empty( $errors['sections'][$tab_key] ) ) ? '<i class="zmop-label-error zmop-error">!</i>' : '';
                $tab_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="zmop-tab-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';

                echo '<li class="bu-tab-item"><a href="#">'. $tab_icon . $section['title'] . $tab_error .'</a></li>';
          ...
          echo '<div class="zmop-content">';
            ...
            echo '<button type="submit" class="build-update-btn metabox_submit"><span class="dashicons dashicons-update-alt"></span></button>';
            $section_key = 0;
            ...
            foreach ( $this->sections as $section ) {
              ...
              echo '<div class="zmop-section hidden'. esc_attr( $section_onload . $section_class ) .'">';

              echo ( $section_title || $section_icon ) ? '<div class="zmop-section-title"><h3>'. $section_icon . $section_title .'</h3></div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  ZMOP::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );

                }

              } else {

                echo '<div class="zmop-no-option">'. esc_html__( 'No data available.', 'zmop' ) .'</div>';
          ...
          echo '<div class="clear"></div>';
            if ( ! empty( $this->args['show_restore'] ) || ! empty( $this->args['show_reset'] ) ) {
              ...
              echo '<div class="zmop-sections-reset">';
              echo '<label>';
              echo '<input type="checkbox" name="'. esc_attr( $this->unique ) .'[_reset]" />';
              echo '<span class="button zmop-button-reset">'. esc_html__( '重置', 'zmop' ) .'</span>';
              echo '<span class="button zmop-button-cancel">取消重置</span>';
              echo '<input type="submit" class="build-update metabox_submit button" value="更新" />';
              echo '</label>';
              echo '</div>';
            }
        echo '</div>';
    ...
    public function save_meta_box( $post_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'zmop_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'zmop_metabox_nonce' ) ) {
        return $post_id;
      }
      ...
      $data = apply_filters( "zmop_{$this->unique}_save", $data, $post_id, $this );

      do_action( "zmop_{$this->unique}_save_before", $data, $post_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $post_id, '_zmop_errors_'. $this->unique, $errors );
        }

      }

      do_action( "zmop_{$this->unique}_saved", $data, $post_id, $this );

      do_action( "zmop_{$this->unique}_save_after", $data, $post_id, $this );
