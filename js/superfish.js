...
jQuery(document).ready(function($){
	$('#all_archives span.bemon').click(function(){
		var $this = $(this);
		var $postList = $this.next('ul.post_list');
		if ($postList.children('li').length > 0) {
			if ($postList.is(':hidden')) {
				$this.addClass('active');
			} else {
				$this.removeClass('active');
			}
			$postList.slideToggle(400);
			return false;
		}
		var year = $this.data('year');
		var month = $this.data('month');
		$this.append('<div class="loading"><div class="loading-spin"></div></div>');
		$.ajax({
			url: be_archives.ajax_url,
			type: 'POST',
			data: {
				action: 'be_get_archives_posts',
				year: year,
				month: month
			},
			success: function(response) {
				$postList.html(response).slideToggle(400);
				$this.find('.loading').remove();
				$this.addClass('active');
			},
			error: function() {
				$this.find('.loading').text(' 加载失败');
			}
		});
		return false;
	});
